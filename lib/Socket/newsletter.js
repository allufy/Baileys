...
loadBase(newsletterWMexQuery, QueryIds)
  ...
  return {
    ...
    // --- PENAMBAHAN FUNGSI BARU UNTUK SUPPORT addidch ---
    newsletterGetId: async (jid) => {
        const result = await newsletterMetadata('JID', jid, 'GUEST');
        return result?.id || jid;
    },
    newsletterRoles: async (jid) => {
        const result = await newsletterMetadata('JID', jid, 'ADMIN');
        return result?.viewer_metadata?.role; // Menghasilkan 'OWNER', 'ADMIN', atau 'SUBSCRIBER'
    },
